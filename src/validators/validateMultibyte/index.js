...
/**
 * Validates if the string contains at least one multibyte (non-ASCII) character.
 * @param {string} str - String to validate
 * @returns {ValidationResult} Validation result object
 */
export async function validateMultibyte(str) {
  if (!isString(str)) {
  ...
  try {
    const res = await executeI18nValidationRule(lang, 'isMultibyte', testStr);
    if (res && res.error === 'invalidRule') {
      return { valid: false, error: 'invalidRule', context: { rule: 'isMultibyte' } };
    }

    const matched = (res && res.valid) || res === true;
    if (matched) return { valid: true };
    return { valid: false, error: 'validateMultibyte' };
  } catch (e) {
