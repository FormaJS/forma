...
describe('validateCreditCard extended', () => {
  ...
  it('bulk card dataset (locale pt-BR)', () => {
    ...
    for (const card in cards) {
      const provider = typeof cards[card] === 'string' ? cards[card].toLowerCase() : undefined;
  ...
  it('provider-specific acceptance and mismatches', () => {
    ...
    // Unknown provider
    expect(validateCreditCard('4111111111111111', { provider: 'no-such-provider' })).toMatchObject({
      valid: false,
